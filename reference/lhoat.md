...
- drty.out:
  ...
  lhoat. Required when `write2disk=TRUE`
...
#      be the most important factor. 
set.seed(123) 
lhoat.dir <- file.path(tempdir(), "LH_OAT")
lhoat(
      fn=non.linear, 
      lower=rep(0, nparam), 
      upper=rep(5, nparam),
      control=list(N=100, f=0.005, write2disk=TRUE,
                   drty.out=lhoat.dir, verbose=FALSE)      
      )

# 2.4) reading ALL the parameter sets used in LH-OAT, and plotting dotty plots     
params <- read_params(file=file.path(lhoat.dir, "LH_OAT-gof.txt"), header=TRUE, skip=0, 
...
set.seed(123) 
lhoat.dir <- file.path(tempdir(), "LH_OAT-normalised")
lhoat(
      fn=ishigami, 
      lower=rep(-pi, nparam), 
      upper=rep(pi, nparam),
      control=list(N=5000, f=0.1, write2disk=TRUE, drty.out=lhoat.dir,
                   verbose=FALSE, normalise=TRUE)         
      )
